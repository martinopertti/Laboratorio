...
<!-- .content-box-blue[ -->
<!-- *(1) Con la data de gapminder crear una nueva variable que divida la expectativa de vida en 3 categorías: "Alta" (mayor o igual a 75 años); "Media" (mayor o igual a 65 y menor a 75 y "Baja" (menor a 65 años)* -->

<!-- *(2) Utilizar count() para crear una tabla con el % de casos en cada categoría de la recodificación de expectativa de vida* -->

<!-- *(3) Recrear la tabla debajo (tabla cruzada de continente y expectativa de vida recodificada, con % por fila):* -->
<!-- ] -->

<!-- ```{r chsadasdas13, echo = FALSE} -->
<!-- d_gap <- d_gap %>%  -->
<!--   mutate(vida_rec = case_when( -->
<!--     lifeExp >= 75 ~ "Alta", -->
<!--     lifeExp >= 65 & lifeExp < 75 ~ "Media", -->
<!--     lifeExp < 65 ~ "Baja" -->
<!--   )) -->
<!-- ``` -->

<!-- ```{r ch1sadsa3, echo = FALSE} -->
<!-- d_gap %>%  -->
<!--   count(continent, vida_rec) %>%  -->
<!--   mutate(n = n/sum(n), .by = continent) %>%  -->
<!--   spread(vida_rec, n) -->
<!-- ``` -->

<!-- .content-box-blue[ -->
<!-- *(4) Crear una tabla con * -->
<!-- ] -->
